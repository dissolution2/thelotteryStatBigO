const dataSet = [
  ...
  function calculateProbability(number, tripletFrequency) {
    if (!tripletFrequency[number]) {
      return 0; // Number hasn't appeared with the triplet.
    }
    const totalFrequency = Object.values(tripletFrequency).reduce((acc, freq) => acc + freq, 0);
    return (tripletFrequency[number] / totalFrequency) * 100; // Probability as a percentage.
  }
  
  // Array of user-defined triplets to analyze.
  const userTriplets = [
    [22, 26, 18],
    [24, 27, 13],
    [18, 21, 22],
  ...
  // Loop through each user-defined triplet and analyze.
  for (const userTriplet of userTriplets) {
    console.log(`Analyzing triplet: [${userTriplet}]`);
  
    // Create an object to store the frequency of numbers that have appeared with the userTriplet.
    const tripletFrequency = {};
  
    // Iterate through the dataset and update the frequency of numbers that appeared with the triplet.
    for (const row of dataSet) {
      const numbers = row.slice(0, 6).sort(); // Sort the first 6 numbers to ensure consistent triplets.
      const lastNumber = row[6];
      if (
        numbers.includes(userTriplet[0]) &&
        numbers.includes(userTriplet[1]) &&
        numbers.includes(userTriplet[2])
      ) {
        if (!tripletFrequency[lastNumber]) {
          tripletFrequency[lastNumber] = 0;
        }
        tripletFrequency[lastNumber]++;
      }
    }
  
    // Find the numbers that have appeared most frequently with the userTriplet.
    const mostFrequentNumbers = [];
    let maxFrequency = 0;
    for (const number in tripletFrequency) {
      const frequency = tripletFrequency[number];
    ...
    const probabilities = mostFrequentNumbers.map((number) => ({
      number,
      probability: calculateProbability(number, tripletFrequency),
    }));
  
    // Sort the probabilities in descending order.
    probabilities.sort((a, b) => b.probability - a.probability);
  
    console.log("Numbers that appeared most frequently with", userTriplet, "are:", mostFrequentNumbers);
