...
//Function to count the frequency of specific number sets in the same row
function countNumberSetFrequency(dataSet, numberSet) {
    let count = 0;

    for (const row of dataSet) {
        if (numberSet.every(num => row.includes(num))) {
            count++;
        }
    }
...
// Define the number sets you want to check
const numberSetsToCheck = [
    [13,22, 26],  // Example: Check how often 22 and 26 are drawn together in the same row
    // Add more number sets to check here
];

// Calculate the frequency of the specified number sets in the same row
const frequencyResults = {};

for (const numberSet of numberSetsToCheck) {
    const setString = numberSet.sort((a, b) => a - b).toString();
    const count = countNumberSetFrequency(dataSet, numberSet);
    frequencyResults[setString] = count;
}

console.log("Frequency of Number Sets in the Same Row:", frequencyResults);