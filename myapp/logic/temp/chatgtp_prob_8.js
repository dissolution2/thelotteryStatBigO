...
function adjustNumberProbabilities(dataSet, number) {
    ...
    for (let i = 1; i <= 48; i++) {
        if (i !== number) {
            ...
            const adjustedProbability = (pairFrequency + 1) / (numberFrequency[number] + 1);
...
function adjustNextNumberProbabilities(dataSet, currentNumber) {
    ...
    // Find the next number in the line with the highest probability
    let nextNumber = null;
    ...
    // If the next number is the same as the current number, we'll pick the second highest probability
    if (nextNumber === currentNumber) {
        ...
        for (let i = 1; i <= 48; i++) {
            if (i !== currentNumber) {
                const currentProbability = parseFloat(currentProbabilities[i] || '0.00');
                if (currentProbability > secondHighestProbability && currentProbability < highestProbability) {
                    secondHighestProbability = currentProbability;
                    nextNumber = i;
                }
            }
        }
...
// Function to select the next n numbers with the highest probabilities
function selectNextNumbers(dataSet, currentNumber, numToSelect) {
    const adjustedProbabilities = adjustNumberProbabilities(dataSet, currentNumber);

    const selectedNumbers = adjustedProbabilities
...
// Function to calculate the frequency and probability in % for selected numbers
function calculateFrequencyAndProbability(dataSet, selectedNumbers) {
    const numberFrequency = calculateNumberFrequency(dataSet);
    const totalRows = dataSet.length;

    const frequencyAndProbability = {};

    for (const num of selectedNumbers) {
        const frequency = numberFrequency[num] || 0;
        const probability = ((frequency / totalRows) * 100).toFixed(2);
        frequencyAndProbability[num] = { frequency, probability };
    }

    return frequencyAndProbability;
}

// Example: Calculate and display adjusted probabilities as percentages for numbers in the same row as 22
const numberToAnalyze1 = 22;
const adjustedProbabilities1 = adjustNumberProbabilities(dataSet, numberToAnalyze1);

console.log(`Adjusted Probabilities for Numbers in the Same Row as ${numberToAnalyze1}:`);
adjustedProbabilities1.forEach((num) => {
    const probability = adjustedProbabilities1[num] || '0.00'; // Default to '0.00' if undefined
    const frequency = calculateNumberFrequency(dataSet)[num] || 0;
    console.log(`Number ${num}: Probability=${probability}%, Frequency=${frequency}`);
});

// Example: Calculate the next number in the line after 22 and 13 based on frequency and probability
const nextNumber = adjustNextNumberProbabilities(dataSet, 13);
console.log(`Next Number in the Line after 22 and 13: ${nextNumber}`);

// Example: Select the next 3 numbers with the highest probabilities
const nextNumbersToSelect = 3;
const selectedNumbers = selectNextNumbers(dataSet, numberToAnalyze1, nextNumbersToSelect);

console.log(`Selected Next Numbers with Highest Probabilities (Top ${nextNumbersToSelect}):`);
selectedNumbers.forEach((num) => {
    console.log(`Number ${num}`);
});

// Example: Calculate frequency and probability in % for the last 3 selected numbers
const frequencyAndProbability = calculateFrequencyAndProbability(dataSet, selectedNumbers);
console.log(`Frequency and Probability for the Last 3 Selected Numbers:`);
for (const num of selectedNumbers) {
    const { frequency, probability } = frequencyAndProbability[num];
    console.log(`Number ${num}: Frequency=${frequency}, Probability=${probability}%`);
}
