function findFrequentNumberSets(dataSet, setSize) {
    const numberSets = new Map();

    // Iterate through each row in the dataset
    for (const row of dataSet) {
        // Generate all possible combinations of 'setSize' numbers from the row
        const combinations = generateCombinations(row.slice(0, 6), setSize);

        for (const combination of combinations) {
            // Sort the combination to ensure consistency
            const sortedCombination = combination.slice().sort((a, b) => a - b);

            if (numberSets.has(sortedCombination.toString())) {
                // If the combination is already in the map, increment its count
                numberSets.set(sortedCombination.toString(), numberSets.get(sortedCombination.toString()) + 1);
            } else {
                // Otherwise, initialize it with a count of 1
                numberSets.set(sortedCombination.toString(), 1);
            }
        }
    }

    // Filter sets with a frequency greater than 1 (adjust as needed)
    const frequentSets = [...numberSets.entries()].filter(([_, count]) => count > 1);

    return frequentSets.map(([numbers, count]) => ({
        numbers: numbers.split(',').map(Number),
        frequency: count,
    }));
}

function generateCombinations(arr, setSize) {
    const combinations = [];
    const indices = Array.from({ length: setSize }, (_, i) => i);

    while (true) {
        const combination = indices.map(index => arr[index]);
        combinations.push(combination);

        // Generate the next combination
        let i;
        for (i = setSize - 1; i >= 0; i--) {
            if (indices[i] !== arr.length - setSize + i) {
                break;
            }
        }
...
const frequentSets = findFrequentNumberSets(dataSet, 6);
console.log("Frequent Sets:", frequentSets);
