const dataSet = [
    ...
    [19,47,41,6,20,8,3,"September 20, 2023"]
      ...
      // Function to calculate the probability of a number being drawn next.
      function calculateProbability(number, pairFrequency) {
        if (!pairFrequency[number]) {
          return 0; // Number hasn't appeared with the pair.
        }
        const totalFrequency = Object.values(pairFrequency).reduce((acc, freq) => acc + freq, 0);
        return (pairFrequency[number] / totalFrequency) * 100; // Probability as a percentage.
      }
      
      // Function to analyze a pair and a next number.
      function analyzePairAndNext(pair, nextNumber) {
        console.log(`Analyzing pair: [${pair}]`);
        const pairFrequency = {};
        let nextNumberFrequency = 0; // Initialize the frequency of the next number.
      
        // Iterate through the dataset and update the frequency of numbers that appeared with the pair.
        for (const row of dataSet) {
          const numbers = row.slice(0, 5); // Use positions 1 to 6.
          const lastNumber = row[5];
        //   console.log('lastNumber: ', lastNumber);
          if (pair.every(num => numbers.includes(num))) {
            if (!pairFrequency[lastNumber]) {
              pairFrequency[lastNumber] = 0;
            }
            pairFrequency[lastNumber]++;
      
            if (lastNumber === nextNumber) {
                console.log('lastNumber: ',lastNumber, ' nextNumber: ', nextNumber);
              nextNumberFrequency++;
            }
          }
        }
      
        console.log("Frequency for each number with the pair:");
        for (let number = 1; number <= 48; number++) {
          const frequency = pairFrequency[number] || 0;
          console.log(`Number ${number}: ${frequency} times`);
        }
      
        // Calculate the probability for each of the most frequent numbers.
        const mostFrequentNumbers = Object.keys(pairFrequency).map(Number);
        const probabilities = mostFrequentNumbers.map((number) => ({
          number,
          probability: calculateProbability(number, pairFrequency),
        }));
      
        // Sort the probabilities in descending order.
        probabilities.sort((a, b) => b.probability - a.probability);
      
        console.log("Numbers that appeared most frequently with", pair, "are:", mostFrequentNumbers);
        console.log("Probabilities for the next best numbers:", probabilities);
      
        console.log(`Frequency of next number ${nextNumber}: ${nextNumberFrequency} times`);
      
        console.log("----------------------------------");
      }
      
      // Example usage:
      const userPair = [22, 26];
      const userNextNumber = 13;
      analyzePairAndNext(userPair, userNextNumber);
