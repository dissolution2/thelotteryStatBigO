function findPerfectRow(dataSet) {
    let maxProbability = 0;
    let perfectRow = [];

    for (let i = 0; i < dataSet.length - 7; i++) {
        let row = dataSet.slice(i, i + 7);
        let lastNumber = row[6][6]; // Extract the last number in the row

        let probability = 1;

        for (let j = 0; j < 6; j++) {
            let currentNumber = row[j][0];
            let nextNumber = row[j + 1][0];

            // Calculate the probability based on frequency
            let frequency = calculateFrequency(dataSet, currentNumber, nextNumber);
...
function calculateFrequency(dataSet, currentNumber, nextNumber) {
    let count = 0;
    for (let i = 0; i < dataSet.length - 1; i++) {
        let row = dataSet[i];
        if (row.includes(currentNumber) && row.includes(nextNumber)) {
            count++;
        }
    }
    return count / (dataSet.length - 1); // Calculate frequency as a probability
}
...
const perfectRow = findPerfectRow(dataSet);
console.log("Perfect Row:", perfectRow);