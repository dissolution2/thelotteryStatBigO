const dataSet = [
  ...
  // Array of user-defined pairs to analyze.
  const userPairs = [
    [22, 26],
    [24, 27],
    [18, 21],
  ...
  // Loop through each user-defined pair and analyze.
  for (const userPair of userPairs) {
    console.log(`Analyzing pair: [${userPair}]`);
  
    // Create an object to store the frequency of numbers that have appeared with the userPair.
    const pairFrequency = {};
    ...
    for (const row of dataSet) {
      ...
      const lastNumber = row[6];
      if (numbers.includes(userPair[0]) && numbers.includes(userPair[1])) {
    ...
    // Display the frequency of each number with the userPair.
    console.log("Frequency for each number with the pair:");
    for (const number in pairFrequency) {
      console.log(`Number ${number}: ${pairFrequency[number]} times`);
    }
  
    // Find the numbers that have appeared most frequently with the userPair.
    const mostFrequentNumbers = [];
    let maxFrequency = 0;
    for (const number in pairFrequency) {
      const frequency = pairFrequency[number];
    ...
    console.log("Numbers that appeared most frequently with", userPair, "are:", mostFrequentNumbers);
